...
class DiscreteAliasSampler{
    ...
    private:
        ...
        std::vector<int> vecInv;
    public:
        void init(std::unordered_map<uint, double>& mapResidue, double gamma, const Graph& graph, Config& config);
        int sample();
        void batchSample(uint64 num, std::queue<uint>& S);
        size_t size();
