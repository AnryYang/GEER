...
void DiscreteAliasSampler::init(std::unordered_map<uint, double>& mapResidue, double gamma, const Graph& graph, Config& config){
    m_n = mapResidue.size();
    veckN.resize(m_n);
    vecInv.resize(m_n);
    vecProb.resize(m_n);
    ...
    for(std::unordered_map<uint, double>::iterator it = mapResidue.begin(); it != mapResidue.end(); ++it){
...
int DiscreteAliasSampler::sample(){
    double prob = mt_drand();
    uint i = xorshift32()%m_n;
    return prob<=vecProb[i]?i:vecInv[i];
}

void DiscreteAliasSampler::batchSample(uint64 num, queue<uint>& S){
